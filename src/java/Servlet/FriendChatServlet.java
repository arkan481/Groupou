...
public class FriendChatServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String message = request.getParameter("friendMessage");
        
        FriendChatModel fcm = new FriendChatModel();
        fcm.setSenderID(senderID);
        fcm.setReceiverID(receiverID);
        fcm.setMessage(message);
        
        FriendChatController fcc = new FriendChatController();
        boolean success = fcc.create(fcm);
        
        if (success) {
            response.sendRedirect("./chat?friend="+receiverID);
        }else {
